...
### Windows button latency

Transcription timing does not measure how quickly recording starts. The former
Windows button path launched a PowerShell command, then another recorder host,
compiled the recorder and waited 300 ms before signalling the indicator.
Three local start-command-to-indicator-marker measurements were 3105, 2839 and
3078 ms. These measurements excluded the mouse hook and window painting.

The Windows trigger compiles capture once in its host and
starts a native capture thread per press. It keeps the microphone closed at
idle and signals the indicator only after waveInStart succeeds. Five direct
warm-path measurements were 158, 15, 23, 20 and 22 ms to the indicator marker
(151, 13, 20, 18 and 19 ms to microphone readiness). The trigger's polling and
the indicator's redraw add time; these are not physical-click-to-pixel results.
Interactive use confirmed faster indicator startup and improved responsiveness.

The release path also launched a fresh PowerShell host per recording. Three
paired runs of one public audio clip took 3769/2416, 4130/2028 and 5220/2206 ms
for a new command process versus an already loaded transcription function.
These timings excluded capture and text delivery. A reusable STA worker now
loads transcription and injection once and processes completed recordings in
order. Three integration runs through that worker took 3566, 2419 and 2424 ms,
with delivery replaced by a scratch-file sink to avoid typing into applications.
They verify the pipeline, not physical-release-to-visible-text latency.

A full application restart through the existing Windows logon task brought the
trigger and fast engine back in 14.3 seconds, with no diagnostic failures or
warnings. Automatic startup and dictation were subsequently confirmed by the
user after an actual Windows reboot; that boot's startup time was not measured.
